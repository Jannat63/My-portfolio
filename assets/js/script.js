...
(function () {
  ...
  if (typedEl) {
    const words = ['Find', 'Discover', 'Choose', 'Remember'];
...
function blogPage(page) {
  ...
  const grid2  = document.getElementById('postGridPage2');
  const pg1btn = document.getElementById('pg1');
  const pg2btn = document.getElementById('pg2');
  ...
  if (page === 1) {
    ...
    if (pg1btn) pg1btn.classList.add('act');
    if (pg2btn) pg2btn.classList.remove('act');
    if (prev) prev.disabled = true;
    if (next) next.disabled = false;
    if (next) next.onclick = () => blogPage(2);
  } else {
    grid1.style.display = 'none';
    if (grid2) { grid2.style.display = ''; }
    if (pg1btn) pg1btn.classList.remove('act');
    if (pg2btn) pg2btn.classList.add('act');
    if (prev) { prev.disabled = false; prev.onclick = () => blogPage(1); }
    if (next) next.disabled = true;
    // Re-trigger reveal animations on page 2 cards
    if (grid2) {
      grid2.querySelectorAll('[data-reveal]').forEach(el => {
        el.classList.remove('in');
        setTimeout(() => el.classList.add('in'), 50);
      });
    }
    window.scrollTo({ top: document.getElementById('postGrid')?.offsetTop - 120 || 0, behavior: 'smooth' });
  }
}
