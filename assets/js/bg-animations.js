...
let transitionProgress = 1; // 0→1 means transitioning in
...
function drawHome(alpha) {
  ...
  // Animated radial glow — top right (orange)
  ...
  // Animated radial glow — bottom left (blue)
...
function drawAbout(alpha) {
  ...
  // Flowing horizontal wave ribbons
  const waves = [
...
function drawExperience(alpha) {
  ...
  for (let s = 0; s < streams; s++) {
    ...
    // Glowing dots moving along streams
...
function drawContact(alpha) {
  ...
  // Static decorative rings at corners
  const t = Date.now() * 0.001;
...
const sectionRenderers = {
  ...
  testimonials: drawAbout,   // soft waves — suits quotes
  contact:      drawContact,
};
...
function render() {
  ...
  if (nextRenderer && transitionProgress < 1) {
    // Cross-fade: draw current fading out, next fading in
